...
mod skel {
    include!(concat!(env!("OUT_DIR"), "/biolat.skel.rs"));
}

use skel::*;

/// Block IO latency histogram tool (equivalent to bcc biolatency).
#[derive(Parser, Debug)]
struct Args {
    /// Duration in seconds to trace before printing and exiting.
    /// If not specified, runs until Ctrl+C.
    #[arg(short, long)]
    duration: Option<u64>,

    /// Run as a Prometheus exporter serving GET /metrics on this address
    /// (e.g. ":9601"), instead of a one-shot trace. Runs until killed.
...
fn main() -> Result<()> {
    ...
    let skel_builder = BiolatSkelBuilder::default();
    ...
    eprintln!("Tracing block device I/O... Hit Ctrl-C to end.");

    // Set up SIGINT handler: just interrupt the sleep, no flag needed.
    unsafe {
...
/// Render the Prometheus text-format exposition for the block IO histogram.
///
...
fn render_metrics(obj: &libbpf::Object) -> String {
    const NAME: &str = "periscope_block_io_latency_seconds";
    ...
    out.push_str(&format!(
        "# HELP {NAME} Block IO completion latency (seconds).\n"
    ));
